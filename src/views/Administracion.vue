<template>
  <div v-if="acceso" class="admin">
    <h2 class="pl-2" style="color: '#9575cd'">Administración</h2>
    <v-card elevation="5">
      <v-card-title>
        <v-text-field
          v-model="search"
...
export default {
  ...
  methods: {
    initialize() {
      var config = {
        ...
        url:
          ...
          process.env.VUE_APP_RECURSO_GET_ADMINISTRADOR,
        headers: {
          version: "1.0.0",
        },
      };
      ...
      this.axios(config)
        .then(function (response) {
          if (response.data.en == 1) {
            global.desserts = response.data.administrador;
            ...
            global.openNotification(
              ...
              2,
              "Sin administradores",
              "Estimado usuario, no se encontraron administradores"
            );
            global.loading = false;
          }
        })
    ...
    editItem(item) {
      ...
      this.editedItem.idRol = { idRol: item.idRol };
    ...
    deleteItemConfirm() {
      ...
      var data = qs.stringify({
        idAdministrador: globalE.editedItem.idAdministrador,
      });
      console.log(data);
      var config = {
        method: "post",
        url:
          process.env.VUE_APP_URL_SERVICIO +
          process.env.VUE_APP_RECURSO_ELIMINAR_ADMINISTRADOR,
        headers: {
          version: "1.0.0",
        ...
        .then(function (response) {
          if (response.data.en == 1) {
            ...
            globalE.openNotification(
              ...
              "Estimado usuario, hubo un error al intentar registrar al administrador"
            );
          }
        })
        .catch(function (error) {
      ...
      if (this.editedIndex > -1) {
        if (this.$refs.form.validate()) {
          ...
          var dataE = qs.stringify({
            ...
            idRol: global.editedItem.idRol.idRol,
            cambioPassword: global.editedItem.cambioPassword
              ? global.editedItem.cambioPassword
              : -1,
            idAdministrador: global.editedItem.idAdministrador,
          });
          console.log(dataE);
          var configE = {
            method: "post",
            url:
              process.env.VUE_APP_URL_SERVICIO +
              process.env.VUE_APP_RECURSO_UPDATE_ADMINISTRADOR,
            headers: {
              version: "1.0.0",
              "Content-Type": "application/x-www-form-urlencoded",
            },
        ...
        if (this.$refs.form.validate()) {
          ...
          var data = qs.stringify({
            ...
            idRol: globalE.editedItem.idRol.idRol,
          });
          console.log(data);
          var config = {
            method: "post",
            url:
              process.env.VUE_APP_URL_SERVICIO +
              process.env.VUE_APP_RECURSO_CREAR_ADMINISTRADOR,
            headers: {
              version: "1.0.0",
              "Content-Type": "application/x-www-form-urlencoded",
            },
          ...
          //var rol = this.items.find((item) => item.id === this.editedItem.rol);
          /*   this.editedItem.nombreRol = rol.rol;
    ...
    loadComboRol() {
      var config = {
        ...
        url:
          process.env.VUE_APP_URL_SERVICIO +
          process.env.VUE_APP_RECURSO_COMBO_ROLES,
        headers: {
          version: "1.0.0",
        },
      };
      ...
      this.axios(config)
        .then(function (response) {
          if (response.data.en == 1) {
            ...
            global.itemsRol = response.data.roles;
          } /*  else {
